...
public struct ZipArchive: Sendable {
    ...
    private func compressedBytes(of entry: Entry) throws -> ArraySlice<UInt8> {
        try bytes[payloadRange(of: entry)]
    }
    ...
    private static func serialise(_ entry: NewEntry, offset: Int) -> (local: Data, central: Data) {
        ...
        local.append(u16: 0)
        local.append(u16: 0) // dos time, date
        ...
        central.append(u16: 0)
        central.append(u16: 0) // extra, comment
        central.append(u16: 0)
        central.append(u16: 0) // disk, internal attrs
    ...
    private static func endOfCentralDirectory(count: Int, directorySize: Int, directoryOffset: Int) -> Data {
        ...
        record.append(u16: 0)
        record.append(u16: 0) // this disk, directory disk
        record.append(u16: count)
        record.append(u16: count)
    ...
    private static func parseCentralDirectory(_ bytes: [UInt8], name: String) throws -> [Entry] {
        ...
        while end >= limit, u32(end) != 0x0605_4B50 {
            end -= 1
        }
...
public enum ZipError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case let .notAnArchive(name): "\(name) is not a zip archive"
        case let .entryNotFound(path): "the archive has no entry at \(path)"
        case let .corrupt(detail): detail
        case let .zip64(name): "\(name) uses Zip64, which OmniTag cannot read yet"
...
private extension Data {
    ...
    mutating func append(u32 value: Int) {
        append(u32: UInt32(value))
    }
}
